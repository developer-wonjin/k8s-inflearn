...
쿠버네티스를 처음 만나면 컨테이너를 다루는 도구라고 생각하게 된다.
그런데 **쿠버네티스는 컨테이너를 직접 다루지 않는다.**

| 다루는 단위 | 이유 |
|---|---|
| **Pod** | 함께 떠야 하는 컨테이너들을 묶어야 배치·수명·네트워크를 한 번에 정할 수 있다 |
| **Label** | Pod이 수십 개가 되면 이름으로는 관리가 안 된다. 조건으로 골라야 한다 |
| **NodeSchedule** | 어느 노드에 올릴지 통제하지 못하면 자원이 한쪽에 몰려 같이 죽는다 |

이 게시글의 세 주제는 따로 놀지 않는다.
**Pod이라는 단위를 만들고 → 라벨로 그것들을 가리키고 → 어느 노드에 둘지 정한다.**
이후 모든 오브젝트가 이 셋 위에 얹힌다.
Service가 Pod을 찾는 것도 라벨이고, 컨트롤러가 개수를 세는 것도 라벨이다.
...
그래서 2-2에는 정리 블록을 두지 않고, 맨 앞에 전제 확인만 적어 두었다.
