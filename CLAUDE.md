...
4. **`## YAML`** — 매니페스트 전문. 아래 3-6절의 **실행 가능한 형태**로 쓴다
...
- README에는 **게시글 전체 수준**으로 쓴다. 절 문서의 내용을 되풀이하지 않고,
  세 주제가 왜 한 게시글에 묶여 있는지를 밝힌다.
  `### 이 문서에서 확인할 것`은 두지 않는다. 이미 「문서 구성」과
  「이 실습에서 확인한 것」 표가 그 역할을 한다.
- 소급 적용은 **2026-09-12에 전체 71개 문서에 완료**했다.
...
### 3-6) YAML은 그대로 실행되는 형태로 쓴다

`## YAML` 섹션의 매니페스트는 ```` ```yaml ````가 아니라 **리소스 하나당
실행 가능한 ```` ```bash ```` 블록 하나**로 쓴다. 복사해서 붙여넣으면 바로 적용된다.

리소스가 여럿이어도 `---` 로 이어 붙이지 않고 **블록을 나눈다.**
한 번에 하나씩 적용하면서 결과를 확인할 수 있어야 한다.

- 여는 줄 주석에 **무엇을 만드는지** 적는다 — `# pv-01 — 1G / ReadWriteOnce`
- `#` 위치는 **YAML 본문의 주석 열에 맞춘다.** 블록 안에서 한 열로 통일된다.
- **적용 대상이 아닌 YAML은 그대로 둔다.** 설명용 조각(`apiVersion` 없는 부분 발췌),
  kubeconfig(`kind: Config`) 등은 ```` ```yaml ````로 남긴다.
- 바꾼 뒤에는 실제로 실행되는지 확인한다.

  ```bash
  clear                                             # 화면 정리 후 시작
  kubectl apply --dry-run=client -f 추출한파일.yaml  # 클러스터를 바꾸지 않고 검증
  ```

> 2026-09-12에 전체 문서의 매니페스트 103개를 이 형태로 바꿨다.
> `kind: Config` 1개와 설명용 조각 9개는 제외했고, 121개 블록을 dry-run으로 확인했다.
