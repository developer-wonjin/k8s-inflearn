...
2. **`## 실습 시작 전 정리`** — 아래 3절의 정리 블록 (선행 조건이 있으면 경고를 먼저)
3. **`## YAML`** — 매니페스트 전문, **라인마다 주석을 열 맞춰** 단다
4. **`## 실습 과정`** — 명령어 블록과 **실제 출력**을 번갈아
5. **`### 검증: ...`** — 원문 서술을 실제로 확인한 결과 (다르면 그 사실을 남긴다)
6. **`### 정리`** — 이 문서에서 만든 리소스 삭제
7. **`## 배운 것`** — 한 줄 요약 목록
